Hi there, I'm Aditya Srivastav! 👋
I am a Computer Science and Engineering student at Shri Ramswaroop Memorial College of Engineering and Management. Currently in my second year, I am passionate about building software that solves real-world problems and fostering a vibrant developer community.

🚀 About Me

Current Focus: Mastering Data Structures, Algorithms, and Web Development.


Leadership: Serving as the Marketing Coordinator for GDG on Campus - SRMCEM, where I help organize hackathons and workshops for over 500 students.

Experience: Previously a Summer Intern at SRDT Pvt. Ltd., where I worked on practical software development and code reviews.


Goal: Seeking software development opportunities to apply my technical skills to innovative projects.

🛠️ Technical Toolkit
Category	Skills
Languages	
Java, Python, JavaScript, HTML5, CSS 

Competencies	
Data Structures & Algorithms, Web Development 

Tools	
Git, GitHub, VS Code, IntelliJ IDEA 

Soft Skills	
Team Collaboration, Project Management, Marketing 







📂 Featured Projects
💻 Student Data Management System
A Full Stack application designed for faculty to securely manage and update student records.

Implemented backend APIs for data processing, validation, and authentication.

Integrated a relational database for efficient and secure data storage.

🛒 Shopping Website
Developed a responsive Frontend with product search and shop-based filtering.

Integrated map-based location selection to enhance the user experience.









🏆 Achievements & Involvement

GDG on Campus: Active community member promoting developer culture and technical events.


Science Exhibition: Recognized for innovative project presentation and scientific research.


Professional Training: Successfully completed structured internship and training programs.








📫 Connect with Me

LinkedIn: linkedin.com/in/adityasrivastav 


Email: adityakrsrivastav2005@gmail.com 


Location: Lucknow, Uttar Pradesh
